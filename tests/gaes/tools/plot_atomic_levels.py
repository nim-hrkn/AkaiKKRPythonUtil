...
def main():
    ...
    for ax, (lo, hi, scale, title) in zip(axes, ((-3200, 0, "symlog", "all core levels (symlog scale)"),
                                                 (-6.0, 0.2, "linear", "shallow core levels, E > -6 Ry (semicore candidates)"))):
        for orb in ORDER:
        ...
        ax.axhline(-1.2, color="gray", lw=0.8, ls="--")
        ax.legend(ncol=8, fontsize=8, loc="lower left")
    axes[1].text(1, -1.15, "E = -1.2 Ry (typical contour bottom for E_F ~ 0)", fontsize=8, color="gray")
