...
for row, (name, keys, runs) in enumerate(PAIRS):
    for col, (d, ew) in enumerate(runs):
        ...
        ax.set_title("%s: ewidth %.4f, %s (itr %s, log10 rms %s), window [%.2f, %.2f]" % (
            name, ew, "converged" if conv else "NOT converged", n, err, e[0], e[-1]), fontsize=9, loc="left",
            color="k" if conv else "darkred")
for ax in axes[-1]: ax.set_xlabel("E - EF (Ry)")
fig.suptitle("SCF convergence flips with ewidth: red = -ewidth (contour bottom), blue = the level concerned (dashed = * valence), gray = other core levels", fontsize=10)
