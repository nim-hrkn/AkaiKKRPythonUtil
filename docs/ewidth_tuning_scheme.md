...
#### 13.4.1 edelt = 1e-5（2026-09-25、ユーザー指定）

go の edelt_init と判定用 dos の edelt_dos を 1e-5（STEP2 は 1e-3 → 1e-4 → 1e-5）にして同じ 7 構造を再計算した（`docs/data/small_primitive_structures_gaes_summary_edelt1e-5.json`、図 `..._dos_edelt1e-5.png`）。

| 構造 | edelt 1e-5: ewidth の経過 | 使ったギャップ (Ry) | edelt 1e-4 |
|---|---|---|---|
| RbCl | 1.2（old） | [−1.82, −1.00] | 1.2 |
| ZnSe | 1.2（old） | [−2.24, −1.01] | 1.2225 |
| InP | 1.2 → 1.0575 → 1.3575 | [−2.24, −1.19] | 1.5325 |
| InAs | 1.2 → 1.0875 → 1.3725 | [−2.24, −1.18] | 1.5475 |
| InSb | 1.2 → 1.0125 | [−1.06, −0.80]（In 4d* の上） | 1.6225 |
| SnSe2 | 1.2 → 1.2375 | [−1.67, −1.04] | 1.2375 |
| Bi2Se3 | 1.2 → 1.2825 | [−1.66, −1.07] | 1.2825 |

- edelt を 1/10 にすると DOS のピークの裾が細くなり、細かい区間（DOS < 1e-3）が両側に 0.1〜0.2 Ry 広がる。同じ閾値でも実質的に判定が緩くなる。
- In 化合物では In 4d* の直上の区間（−1.06〜−0.80）が幅 eth を超えて最初の候補になる（1.06〜1.09）。InP / InAs はその ewidth の go で 4d が動いて区間が消え、4d 帯の下の 1.36〜1.37 に落ち着く。InSb は 1.0125（下限 1.0 の直上）で 4d* の上に留まる。edelt 1e-4 の 1.53〜1.62 とは別の解で、**どちらを採るかは edelt と閾値の組で決まる**。閾値は 2019 年の edelt 1e-4 に合わせた値なので、edelt を変えるなら dosth2 も見直す必要がある。

### 13.5 RbSeBiFe fcc（HEA4、edelt 1e-3、2026-09-26、ユーザー指定）

単一サイト CPA、akaikkr ビルド、mag、bzqlty 10、Method 2、[1.0, 2.0]、原子あたり判定。**go の edelt は 1e-3、判定用 dos の edelt は 1e-4**（判定用の dos は edelt 1e-4 まで行うのが規則。edelt 1e-3 の dos はギャップの底が持ち上がる）。図 `docs/data/hea_RbSeBiFe_fcc_gaes_dos.png`。

| ewidth_init | 経過 | 結果 |
|---|---|---|
| 1.2 | go 未収束（500 反復、rms 0.59） | `ewidth_fail`: 未収束のポテンシャルの DOS には −2.2〜−0.5 Ry に鋭いピークが多数出て、DOS < 2e-2 の区間が 7 つ（幅 0.06〜0.21）に割れ、eth 0.3 を超える区間が無い（`..._init1.2_unconverged.png`） |
| 1.4 | go 収束（268 反復）→ `new` 1.4194 | 粗い区間 [−1.57, −0.96]（Bi 5d と Rb 4p* / Se 4s* / Bi 6s* の間）、細かい区間 [−1.48, −1.41] で −1.40 はその外。次の go（1.4194）が 20 反復目で `***err in reconf...ecor not found` で停止 → status `error`。akaikkr ビルドの reconf 停止は Hf 以外でも起きる |
| 1.55 | go 収束（210 反復）、`old` | **finished、ewidth 1.55**。細かい区間 [−1.60, −1.48]（粗い区間 [−1.63, −1.33]）。E_F 0.155 / 0.162 Ry（up / down）、全エネルギー −14106.7036 Ry、全モーメント 0.896 μB（Fe 3.65、Se 0.11、Bi −0.08、Rb −0.01） |

準位（E − E_F）: Rb 4s −2.07（core）、Bi 5d −1.67（core）、Rb 4p* −0.91、Se 4s* −0.84、Bi 6s* −0.76（valence）。積分路の下端 −1.55 は Bi 5d の直上、s / p の semicore 群の下にある。

- 1.2 の失敗は「ギャップが無い」のではなく go が収束しなかったことによる。初期値を Bi 5d と semicore 群の間（1.4〜1.55）に置けば収束する。§14 の準位表から初期値を決めるとこの系では 1.4〜1.5 が出る（Bi 5d −1.73、Rb 4p* −0.94 → 許容区間 [1.14, 1.53]）。
- 1.4194 の reconf 停止: 積分路の下端 −1.42 が細かい区間 [−1.48, −1.41] の縁で、SCF 中に準位が下端をまたいだと考えられる（§13.1 と同じ機構）。GAES は go の失敗を `error` で止めるので、次の候補（1.55 側）を試す再試行が要る（未実装。§13.1 の cpa2021v01 への切替も可）。
- 収束した 1.55 の DOS でも Bi 5d と semicore 群の間に鋭いピークが残る。成分・l・スピン別の DOS（out_dos.log の `DOS of component`）で帰属すると、−1.17 / −1.19 Ry（up / down、高さ 4300 / 38）と −0.63 / −0.56 Ry（225 / 19000）は **Rb の s チャネル**で、core 準位表のどの準位にも対応しない（Rb 4s は −2.10、Rb 4p* は −0.93、Se 4s* −0.83、Bi 6s* −0.75、Bi 5d −1.70 はそれぞれ PDOS のピークと一致）。高さが 10³〜10⁴ states/Ry でスピンごとに位置が違うことから、Rb の s チャネルの数値的な極（単一サイト CPA の Rb の大きな球で s の共鳴が異常に鋭くなる）と考えられ、物理的な semicore ではない。この極が粗い区間 [−1.57, −0.96] を −1.17 で分断し、1.4 では細かい区間が [−1.48, −1.41] に縮む原因になっている。1.2 の未収束の DOS ではさらに多くの極が現れていた。判定は DOS の値だけで行うため、こうした極も「DOS が高い」として扱う（規則どおり）。結果 `docs/data/hea_RbSeBiFe_fcc_gaes_summary.json`。
- **dos の edelt を 1e-3 にした場合**（同じ 1.55 のポテンシャルから dos だけ再計算、図 `docs/data/hea_RbSeBiFe_fcc_dos_edelt_1e-4_vs_1e-3.png`）: ピークがなまって Rb の s 極の裾がギャップに広がり、粗い区間は [−1.60, −1.48]（幅 0.12 < eth）だけになって判定は `fail`（1e-4 では [−1.63, −1.33] の粗い区間と [−1.60, −1.48] の細かい区間で `old`）。極のまわりでは DOS が負になる点もあり、1e-3 では −0.2 states/Ry に達する（1e-4 では −0.02）。edelt 1e-3 の dos は判定に使えない、という §7 の規則（判定用 dos は 1e-4 まで行う）の実例。
